    ...
    throws ServletException, IOException {
       PrintWriter pr = response.getWriter();
        try {
            ...
            int totalSearch = new userDAO().getTotalAccountStudent();
            int totalPage = totalSearch / PAGE_SIZE;
            if (totalSearch % PAGE_SIZE != 0) {
                totalPage += 1;
            }
            userDAO ud = new userDAO();
            //request.getSession().setAttribute("listUser", ud.getAllUser());
            ArrayList<User> listUserByPagging = ud.getListStudentAndPaging(page, PAGE_SIZE);
            request.getSession().setAttribute("listStudent", listUserByPagging);
          ...
          //  request.getSession().setAttribute("search_url", "search_user");
            request.setAttribute("pagination_url", "student-list?");
            pr.print(listUserByPagging.size());
            pr.print(ud.getAllUser().size());
            pr.print(listUserByPagging.size());
